<template>
  <q-page class="bg-grey-1 q-pb-xl">
    ...
    <div v-else>
      ...
      <div class="relative-position" style="height: 400px; max-height: 50vh; overflow: hidden">
        <q-img :src="newsItem.image" class="fit">
          <template v-slot:error>
            <div class="absolute-full flex flex-center bg-grey-3 text-grey-6">No Image</div>
          </template>
        </q-img>
        <div
          class="absolute-bottom text-white q-pa-xl"
          style="background: linear-gradient(transparent, rgba(0, 0, 0, 0.9))"
        >
          <div style="max-width: 900px; margin: 0 auto">
            <q-chip color="accent" text-color="dark" class="text-weight-bold q-mb-sm">{{
              newsItem.category
            }}</q-chip>
            <div class="text-h3 text-weight-bold q-mb-sm" style="line-height: 1.3">
              {{ newsItem.title }}
            </div>
          </div>
        </div>
      ...
      <!-- Main Content -->
      <div
        class="q-pa-md q-pa-md-xl q-mt-md"
        style="
          max-width: 900px;
          margin: 0 auto;
          background: white;
      ...
      >
        <!-- Metadata -->
        <div
          class="row items-center q-mb-lg text-grey-7"
          style="border-bottom: 1px solid #eee; padding-bottom: 16px"
        >
